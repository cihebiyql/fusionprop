...
try:
    ...
    pass  # 已经设置过
# 提供一个选项，将所有序列提取的特征全部保存在本地，然后再进行训练。
# 禁用不必要的警告
...
class TrainingConfig:
    ...
    def __init__(self):
        ...
        # 特征磁盘存储选项
        self.save_features_to_disk = False  # 是否将提取的特征保存到磁盘
        self.load_features_from_disk = False  # 是否从磁盘加载特征
        self.features_disk_dir = None  # 特征磁盘存储目录
...
class FeatureExtractor:
    ...
    def save_all_features_to_disk(self, features_dict, features_dir, dataset_name="train"):
        """将所有序列的特征保存到磁盘"""
        if self.logger:
            self.logger.info(f"开始将{len(features_dict)}个序列的特征保存到磁盘: {features_dir}")

        # 确保目录存在
        os.makedirs(features_dir, exist_ok=True)

        # 保存特征
        feature_file = os.path.join(features_dir, f"{dataset_name}_features.pt")
        try:
            torch.save(features_dict, feature_file)
            if self.logger:
                self.logger.info(f"特征已成功保存到: {feature_file}")
            return True
        except Exception as e:
            if self.logger:
                self.logger.error(f"保存特征到磁盘失败: {str(e)}")
            return False

    def load_all_features_from_disk(self, features_dir, dataset_name="train"):
        """从磁盘加载所有序列的特征"""
        feature_file = os.path.join(features_dir, f"{dataset_name}_features.pt")

        if not os.path.exists(feature_file):
            if self.logger:
                self.logger.warning(f"特征文件不存在: {feature_file}")
            return None

        try:
            if self.logger:
                self.logger.info(f"从磁盘加载特征: {feature_file}")
            features_dict = torch.load(feature_file)
            if self.logger:
                self.logger.info(f"成功加载{len(features_dict)}个序列的特征")
            return features_dict
        except Exception as e:
            if self.logger:
                self.logger.error(f"从磁盘加载特征失败: {str(e)}")
            return None
...
class ModelTrainer:
    ...
    def pre_extract_all_features(self):
        ...
        # 检查是否从磁盘加载特征
        if self.train_config.load_features_from_disk and self.train_config.features_disk_dir:
            self.log(f"尝试从磁盘加载特征: {self.train_config.features_disk_dir}")

            # 加载训练集特征
            self.train_features = self.feature_manager.load_all_features_from_disk(
                self.train_config.features_disk_dir, "train")

            # 加载测试集特征
            self.test_features = self.feature_manager.load_all_features_from_disk(
                self.train_config.features_disk_dir, "test")

            # 如果成功加载了特征，直接返回
            if self.train_features is not None and self.test_features is not None:
                self.log("成功从磁盘加载预提取的特征")
                return self.train_features, self.test_features
            else:
                self.log("从磁盘加载特征失败，将重新提取特征")
        ...
        # 如果需要保存到磁盘
        if self.train_config.save_features_to_disk and self.train_config.features_disk_dir:
            self.log(f"将提取的特征保存到磁盘: {self.train_config.features_disk_dir}")

            # 保存训练集特征
            train_saved = self.feature_manager.save_all_features_to_disk(
                self.train_features, self.train_config.features_disk_dir, "train")

            # 保存测试集特征
            test_saved = self.feature_manager.save_all_features_to_disk(
                self.test_features, self.train_config.features_disk_dir, "test")

            if train_saved and test_saved:
                self.log("所有特征已成功保存到磁盘")
            else:
                self.log("特征保存到磁盘过程中出现错误")
...
def parse_args():
    ...
    # 特征磁盘存储相关参数
    parser.add_argument('--save_features_to_disk', action='store_true', default=False,
                        help='是否将提取的特征保存到磁盘')
    parser.add_argument('--load_features_from_disk', action='store_true', default=False,
                        help='是否从磁盘加载特征')
    parser.add_argument('--features_disk_dir', type=str, default=None,
                        help='特征磁盘存储目录')
...
def main():
    ...
    # 特征磁盘存储配置
    config.training_config.save_features_to_disk = args.save_features_to_disk
    config.training_config.load_features_from_disk = args.load_features_from_disk
    ...
    if args.features_disk_dir:
        config.training_config.features_disk_dir = args.features_disk_dir
    else:
        # 如果未指定，使用运行目录下的features子目录
        config.training_config.features_disk_dir = os.path.join(run_dir, "features")

    if args.save_features_to_disk or args.load_features_from_disk:
        logger.info(f"特征磁盘存储目录: {config.training_config.features_disk_dir}")

    if args.save_features_to_disk:
        logger.info("启用特征保存到磁盘")
        os.makedirs(config.training_config.features_disk_dir, exist_ok=True)

    if args.load_features_from_disk:
        logger.info("启用从磁盘加载特征")
        if not os.path.exists(config.training_config.features_disk_dir):
            logger.warning(f"特征目录不存在: {config.training_config.features_disk_dir}")

    # 更新GPU设置
    if torch.cuda.is_available():
        gpu_count = torch.cuda.device_count()
        logger.info(f"检测到 {gpu_count} 个GPU")
        feature_device = torch.device("cuda:3")
        train_device = torch.device("cuda:3")
        logger.info("单GPU环境: 特征提取和训练将共用同一个GPU (cuda:3)")
        # if gpu_count == 1:
